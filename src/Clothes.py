...
class Clothes:
    ...
    def detect_from_frame(frame):
        ...
        clothes_results = Clothes.clothes_model(frame)
        clothes_results = clothes_results.pandas().xyxy[0]
        clothes = []
        for _, row in clothes_results.iterrows():
            if row['confidence'] > 0.5:
                # append clothes name and its position
                clothes.append((row['name'], (int(row['xmin']), int(row['ymin']), int(row['xmax']), int(row['ymax']))))
